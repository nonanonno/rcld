module lifecycle_msgs.srv.humble;
version (humble)
{
    ...
    struct GetAvailableStates_Request
    {
        ...
        static void convert(in GetAvailableStates_Request src,
                ref GetAvailableStates_Request.CType dst)
        {
            dst.structure_needs_at_least_one_member = src.structure_needs_at_least_one_member;
        }

        static void convert(in GetAvailableStates_Request.CType src,
                ref GetAvailableStates_Request dst)
        {
            dst.structure_needs_at_least_one_member = src.structure_needs_at_least_one_member;
        }
    }
    ...
    struct GetAvailableStates_Response
    {
        ...
        static void convert(in GetAvailableStates_Response src,
                ref GetAvailableStates_Response.CType dst)
        {
            ...
            {
                lifecycle_msgs.msg.State.convert(src.available_states[i],
                        dst.available_states.data[i]);
            }
        }

        static void convert(in GetAvailableStates_Response.CType src,
                ref GetAvailableStates_Response dst)
        {
            dst.available_states.length = src.available_states.size;
            foreach (i; 0U .. dst.available_states.length)
            {
                lifecycle_msgs.msg.State.convert(src.available_states.data[i],
                        dst.available_states[i]);
            }
        }
    }
